        ...
        if jsonContent['msg'] == "Y" or jsonContent['msg'] == "y":
            if session is None:
                ...
                if u and u.progress == 33:
                    url = "http://backend:5002/timeslot"
                    payload = {
                        'token': session,
                    ...
                    if r.status_code == 409:
                        if jsonContent['error'] == "You cannot make the same booking.":
                            ...
                            url = "http://backend:5001/dentists/id/" + str(u.chosen_doctor)
            ...
            if a == 599:
                ...
                response = requests.get(url="http://backend:5001/dentists", timeout=2)
                resp_dict = response.json()
                ...
                url = "http://backend:5001/dentists/name/" + doctor_name
                response = requests.get(url=url, timeout=2)
                ...
                url_2 = "http://backend:5002/timeslot"
                ...
                resp_dict = response.json()
                if session is None:
                    url = "http://backend:5001/dentists/id/" + str(resp_dict["dentist_id"])
                    response = requests.get(url=url, timeout=2)
                    resp_dict = response.json()
                    doctor_name = resp_dict["name"].split(' ')[1]
                    msg_1 = "You're now ready to book with Dr " + doctor_name + " " + \
